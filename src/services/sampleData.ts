 ...
 */
import { completeCheckup, listCheckupsForDevice } from '@/repositories/checkups';
import { createDeviceWithInitialData } from '@/repositories/devices';
import {
...
export async function loadSampleData(): Promise<void> {
  ...
  const start1 = addMonths(today, -14);
  const device1 = await createDeviceWithInitialData(
    {
  ...
  const start2 = addMonths(today, -2);
  const device2 = await createDeviceWithInitialData(
    {
